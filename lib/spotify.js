...
function loadTrackCache() {
  if (!fs.existsSync(TRACK_CACHE_FILE)) return {};
  try {
    return JSON.parse(fs.readFileSync(TRACK_CACHE_FILE, 'utf8'));
  } catch (err) {
...
function saveTrackCache(cache) {
  fs.writeFileSync(TRACK_CACHE_FILE, JSON.stringify(cache, null, 2));
}
